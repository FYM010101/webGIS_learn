<template>
    <div class="XZQHComponent-pannel" v-show="this.$store.getters._getDefaultXZQHComponentVisible">
        ...
        <div class="XZQH-select-pannel">
            <span class="XZQH-select-lable">省份：</span>
            <el-select
    ...
    methods: {
        async getProvinceData() {
            ...
            const queryTask = new QueryTask({
                url: 'https://services3.arcgis.com/U26uBjSD32d7xvm2/arcgis/rest/services/XZQHPravince_Mercator/FeatureServer/0',
            });
            let query = new Query();
        ...
        async getCityAndCountyData(value) {
            ...
            const [QueryTask, Query] = await loadModules(['esri/tasks/QueryTask', 'esri/tasks/support/Query'], options);
            const queryTask = new QueryTask({
                url: 'https://services3.arcgis.com/U26uBjSD32d7xvm2/arcgis/rest/services/XZQHCity_Mercator/FeatureServer/1',
            });
            let query = new Query();
            query.returnGeometry = false;
            query.outFields = ['*'];
                ...
                Promise.all(
                    currentCityData.map(async (item2) => {
                        ...
                        const queryTask2 = new QueryTask({
                            url: 'https://services3.arcgis.com/U26uBjSD32d7xvm2/arcgis/rest/services/XZQHConty_Mercator/FeatureServer/2',
                        });
        ...
        async handleItemClick(val, type) {
            ...
            if (type === 'city') {
                ...
                serverUrl =
                    'https://services3.arcgis.com/U26uBjSD32d7xvm2/arcgis/rest/services/XZQHCity_Mercator/FeatureServer/1';
            } else if (type === 'county') {
                code = val.toString().substring(0, 6);
                serverUrl =
                    'https://services3.arcgis.com/U26uBjSD32d7xvm2/arcgis/rest/services/XZQHConty_Mercator/FeatureServer/2';
            }
        ...
        closeXZQHPannel() {
            const currentVisible = this.$store.getters._getDefaultXZQHComponentVisible;
